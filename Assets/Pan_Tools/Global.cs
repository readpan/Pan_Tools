...
namespace Pan_Tools
{
    public abstract class Global : MonoBehaviour
    {
        ...
        /// <returns></returns>
        public static IEnumerator DoSomethingInXSecond(Action action, float time)
        {
        ...
        public static IEnumerator SetVisibleInXSecond(GameObject g, bool visible, float time)
        {
            yield return new WaitForSeconds(time);
            g.SetActive(visible);
        }
        ...
        /// <returns></returns>
        public static bool LoadTxtFromResources(string path, out string saveTxt)
        {
            try
            {
                TextAsset textAsset = Resources.Load(path) as TextAsset;
                saveTxt = textAsset.text;
                return true;
            }
            catch
            {
                saveTxt = string.Empty;
                return false;
            }
        }
